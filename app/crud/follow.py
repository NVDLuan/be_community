...
class CRUDFollowing(CRUDBase[Following, FollowingCreate, FollowingCreate]):
    ...
    def get_following_by_userto_fr(self, db:Session, follower_id: str, user_to:str):
        return db.query(self.model).filter(and_(self.model.id_follower == follower_id, self.model.id_user_to == user_to)).first()

    def get_count_following(self, db: Session, user_id: str):
        query = db.query(self.model).join(Follower, self.model.id_follower == Follower.id).filter(Follower.id_user_fr == user_id)
        return query.count()
